...
const emptyRetrieve: RetrieveLessonsFn = () => [];
...
test("prepareInjection skips when private mode is active", () => {
  const state = createInjectionState();
  const result = prepareInjection(state, privateToggles(), {
    sessionId: "s1",
    query: "test query",
    projectId: "p1",
...
test("prepareInjection skips when retrieval is disabled", () => {
  const state = createInjectionState();
  const result = prepareInjection(state, disabledRetrievalToggles(), {
    sessionId: "s1",
    query: "test query",
    projectId: "p1",
    now: "2026-09-06T00:00:00.000Z",
...
test("prepareInjection skips on empty query", () => {
  const state = createInjectionState();
  const result = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    query: "",
    projectId: "p1",
    now: "2026-09-06T00:00:00.000Z",
...
test("prepareInjection skips on whitespace-only query", () => {
  const state = createInjectionState();
  const result = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    query: "   \n\t  ",
...
test("prepareInjection returns empty when no lessons match", () => {
  const state = createInjectionState();
  const result = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    query: "some query",
...
test("prepareInjection stores pending when lessons are found", () => {
  const state = createInjectionState();
...
test("prepareInjection suppresses on ambiguous correlation and clears stale pending", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];

  const first = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    messageId: "m1",
    query: "first query",
    projectId: "p1",
  ...
  const second = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    messageId: "m2",
    query: "second query",
    projectId: "p1",
...
test("prepareInjection returns failed when retrieval throws", () => {
  const state = createInjectionState();
  const failingRetrieve: RetrieveLessonsFn = () => {
...
test("prepareInjection captures non-Error throws as strings", () => {
  const state = createInjectionState();
  const failingRetrieve: RetrieveLessonsFn = () => {
...
test("applyInjection merges into existing primary system block", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];
  prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    query: "test",
    projectId: "p1",
    now: "2026-09-06T00:00:00.000Z",
...
test("applyInjection pushes block when system array is empty", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];
  prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
...
test("applyInjection clears pending state after application", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];
  prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
...
test("applyInjection returns receipt with correct counts", () => {
  const state = createInjectionState();
  const lessons = [
    makeLesson("L1", {
      scope: "project",
...
test("concurrent sessions do not cross-contaminate", () => {
  const state = createInjectionState();
...
test("clearPendingInjection removes a pending entry", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];
  prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
...
test("prepareInjection without messageId stores undefined messageId", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];
  const result = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
...
test("prepareInjection preserves system block with multiple existing entries", () => {
  const state = createInjectionState();
  const lessons = [makeLesson("L1")];
  prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
...
test("full round-trip: prepare then apply produces correct injection", () => {
  const state = createInjectionState();
  const lessons = [
    makeLesson("L1", {
      scope: "project",
  ...
  const prepareResult = prepareInjection(state, enabledToggles(), {
    sessionId: "s1",
    messageId: "m1",
