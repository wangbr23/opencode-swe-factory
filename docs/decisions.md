...
## 2026-09-07 — Automatic lesson proposals consume their evidence tasks via a provenance marker, and only verification-category successes count as repetition

**Status:** Accepted

**Context:** T59 (design line: automated outcomes may trigger a successful-method proposal only after repeated strong evidence, never from silence) left two questions open: what state prevents the evaluator from re-proposing over the same successes, and which objective signals demonstrate a *verified* method. Approval fatigue is a named design risk, so both answers had to stay conservative without adding a schema surface.
...
## 2026-09-09 — Outcome-failure reclassification supersedes with corrected classification; withdrawal unlinks the execution, and only humans can withdraw

**Status:** Accepted

**Context:** T90 adds the CLI surface for the design's false-attribution risk (design line 180: tool failures, environment problems, or agent prompts may be blamed on a model). Today every effective execution-linked reliability signal counts against its model regardless of cause, and `failureKind` metadata is written only by automatic capture (`classifyFailureKind`) — a misclassified failure (a user cancellation that looks like a `command` failure, an auth expiry that looks like a provider 500) had no correction path. Signals are immutable rows, and aggregation consumes only effective (non-superseded), execution-linked signals.

**Decision:** `reclassifyOutcomeFailure` (src/core/evidence/outcome-reclassification.ts) supersedes a recorded failure with a corrected signal — no new tables, columns, or aggregation changes. Scope is the two objective outcome kinds (`tool-outcome`, `assistant-finish`) with value 0; feedback and correction-lesson signals keep their own human controls and are not reclassifiable. `--failure-kind` corrects the classification within the `ToolFailureKind` vocabulary, preserving the original observation metadata and recording the previous kind as `reclassifiedFailureKind`. `--not-model-caused` withdraws the failure's attribution to the executing model by recording the correction *without* the execution link, so the existing aggregation join excludes it and the estimate recomputes — a human override, deliberately narrower than changing aggregation to auto-exclude not-model-caused kinds, which would silently reclassify already-recorded evidence and belongs behind the T93/T22 evidence review. Withdrawal is reversible: the correction stores `originalExecutionId` provenance, and reclassifying a withdrawn signal without the flag restores the (membership-validated) link, so the flag is explicit in both directions.

**Consequences:** The audit trail stays complete (original immutable, `Retracts` links, corrected metadata) while model evidence recomputes immediately. No free-text reason is accepted — free text in signal metadata would export unredacted (T56's no-notes precedent), and the kind vocabulary plus supersession chain is the record. Assistant-finish failures that were never reclassified still count in reliability per design line 208; only an explicit human reclassification changes that. A future class-aware aggregation (e.g. discounting `provider`/`cancellation` failures automatically) remains a deliberate post-T93 scoring change, and reclassification data makes it retroactively computable.

## 2026-09-10 — Lesson overrides resolve through one human-gated core transaction; proposal-time semantic detection is additive and fail-open

**Status:** Accepted ([design](designs/2026-09-10-agent-driven-lesson-supersession.md))

**Context:** A live override case (candidate `3c099fc7` replacing lesson `e0a6ec7d`) showed the adapter exposes no way for the agent to supersede — the override was executed via a one-off script against `resolveOverlap`. Three structural gaps surfaced: `resolveOverlap` supersedes and rejects in two separate transactions (a mid-review expiry would leave the old lesson replaced while the candidate stays approvable); `detectLessonDuplicatesAndConflicts` is lexical-only, so a contradiction phrased differently from a months-old lesson can go undetected; and the approval card shows only a title line per overlap, forcing the human to judge replacement from memory. An independent Codex review further verified that `createLessonEmbeddingIndexer` has no production caller, so every semantic channel reads an empty index in normal installs.

**Decision:** Resolution becomes one core operation (`resolvePendingLessonOverlap`) that loads the stored candidate draft and validates expiry, exact scope/project match, caller-project authorization, and secret-acknowledgment parity before superseding the target and rejecting the candidate inside a single transaction. The OpenCode tool `swe_factory_resolve_overlap` is a thin wrapper passing the session's `projectId`; it inherits the commit tool's private-mode posture (no gate) because it reviews an already-scanned, already-persisted candidate and processes no new task text. Cross-project authorization applies to the new tool only — the existing commit tool's posture is unchanged until a deliberate follow-up. The approval card renders bounded body previews between strong delimiters, offers Supersede only for same-scope same-project targets, and renders semantic-only matches as a new `related` relation (own cutoff constant) rather than reusing textual-overlap conflict classification. Detection unions lexical and semantic candidates in core with the injectable embed function, non-blocking and fail-open to lexical-only — model load never delays a proposal, and lexical detection remains the guaranteed floor. The adapter must wire the existing injectable embedding indexer (schedule after commit/resolution and at session start); a CLI `maintenance` command renders the maintenance digest the background diagnostic points at but no command prints today.

**Consequences:** Overrides happen in the same approval interaction where the contradiction is on screen, with no reliance on agent or human memory. The transaction boundary makes partial supersession impossible. Semantic detection coverage is probabilistic (cold start, artifact absence, unfired indexer), so the maintenance digest remains the backstop for contradictions no proposal ever surfaces. `related` is a new relation-vocabulary entry that card consumers must handle. Tombstoning via supersede-with-pointer remains the only way to retire an already-approved lesson (scope is immutable, no per-lesson delete), which keeps per-lesson deactivate/delete on the post-V1 list.
